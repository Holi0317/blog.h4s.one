...
First let's give a bit of background to this. We self hosting and uses [AWS ECS]
on [fargate] for deploying both temporal cluster and worker. The persistent
store is backed by a postgres database, while the visibility store is in the
same postgres cluster, in a separate database.
...
larger cache disk instead of persistence storage.

Because of the stateless design, we can, in theory, easily scale the cluster.
However can't really do per-service scaling because we are deploying in monolith
mode. We choose that for easier management and planing to use vertical scaling
until we hit some real limit.
...
Each workers are pull-based. This means we can just spin up more workers,
connect to the cluster and we now get more capacity for all our workloads.

## The meh: Documentation and docker images

I have to make the expectation clear first: Temporal the company is not a
...
I guess this is how most ECS users deal with installing new projects. Given that
it's AWS's propriety playground, it makes little sense for most projects to even
supports them . Just by developing them requires you pay some bucks to aws,
...
you will probably want `auto-setup` instead of `server` for database
bootstraping script in it.
...
machine management in the past.
